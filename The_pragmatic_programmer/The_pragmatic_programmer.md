...
- 元程序设计
  ...
  再多的天才也无法胜过对细节的专注。细节会弄乱我们整洁的代码，所以要把细节赶出去，让代码变得可配置。
  
  要配置，不要集成。让系统变得可配置。
  
  将抽象放进代码，细节放进元数据。为一般情况编写程序，把具体情况放在别处 -- 在编译的代码库之外。
  
  Unix纯文本的威力。
  
- 时间耦合
  
  容许并发，考虑接触时间或者次序上的依赖，这样可以获得灵活性并且减少很多依赖。
...
- 它只是视图
  ...
  模块（或类）的一个好定义就是，它具有单一的定义良好的责任。
  
  发布/订阅机制。
  
  Model - View - Controller，既让模型与表示模型的GUI分离，也让模型与管理视图的控件分离。
...
- 注重实效的团队
  ...
  不要留破窗户、煮青蛙、交流、不要重复你自己、正交性、自动化、知道何时停止绘画
